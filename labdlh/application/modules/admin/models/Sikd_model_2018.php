...
class Sikd_model_2018 extends CI_Model
{		
	private $db_sikd_2018;
	
	public function __construct()
 	{
 	 	parent::__construct();        
		$this->db_sikd_2018 = $this->load->database('default_sikd_2018', TRUE);		
 	}		
function kgtn_dpa_2018($skpd,$limit,$offset)
	{
	$query = $this->db_sikd_2018->query("SELECT
...
public function get_kgtn_dpa($kode)
	{
		$this->db_sikd_2018->select('dpa_skpd_kgtn.id_dpa_skpd_kgtn,sikd_kgtn.id_sikd_kgtn as kode_kgtn,sikd_kgtn.nm_kgtn,dpa_skpd_kgtn.nm_subkegiatan');
		$this->db_sikd_2018->from('dpa_skpd_kgtn');
		$this->db_sikd_2018->join('sikd_kgtn','sikd_kgtn.id_sikd_kgtn = dpa_skpd_kgtn.sikd_kgtn_id','LEFT');
		$this->db_sikd_2018->where('dpa_skpd_kgtn.id_dpa_skpd_kgtn',$kode);
		$query = $this->db_sikd_2018->get();
		if($query->num_rows()==1)
		{
			return $query->row();
		}
		...
		{
			$fields = $this->db_sikd_2018->list_fields('dpa_skpd_kgtn');
	...
	{
		$dpa_query = $this->db_sikd_2018->query("SELECT id_dpa_dpa FROM dpa_dpa WHERE id_dpa_dpa = $kode");
		return $dpa_query;
	}
	public function get_kode_dpakgtn($kode)
	{
		$dpa_query = $this->db_sikd_2018->query("SELECT * FROM dpa_skpd_kgtn WHERE id_dpa_skpd_kgtn = $kode GROUP BY id_dpa_skpd_kgtn");
		return $dpa_query;
	}
	function get_field_table($table)
	{
			$fields = $this->db_sikd_2018->list_fields($table);
			$all_obj = new stdClass;
			foreach ($fields as $field)
			{
				$all_obj->$field = '';
