  ...
  <table class="table table-bordered table-striped">
    ...
    <?php foreach($items as $row) :?>
      <tr>
        ...
        <td>
          <?php if($row->file <> ""){ ?>
          <a href="<?php echo base_url().'file/kegiatan/'.$row->file ?>" class="btn btn-xs btn-success" title="Download" target="_blank">Download</a>
          <?php } ?>
        </td>
        <td>
          <a href="javascript:void(0)" onclick="ubah('<?php echo $row->id?>')" class="btn btn-xs btn-warning" title="Ubah"> <i class="fa fa-pencil"></i></a>
          <a href="javascript:void(0)" onclick="hapus('<?php echo $row->id?>')" class="btn btn-xs btn-danger" title="Hapus"> <i class="fa fa-trash-o"></i></a>
          <a href="internal/kegiatan/ubah_hasil/<?php echo $row->id; ?>" class="btn btn-success btn-xs" title="Hasil Kegiatan"><i class="fa fa-clone"></i></a>
        </td> 
      </tr>
    <?php endforeach; ?>
  </table>
