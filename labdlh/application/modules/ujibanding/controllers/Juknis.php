...
class Juknis extends CI_Controller {
	private $limit = 10;
	function __construct()
	{
		...
		$this->load->model(array('ujibanding/juknis_model'));
	...
	public function index() {
		...
		$this->template->title = 'Juknis';
		$data['menu'] = 'juknis';
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('juknis/v_index',  isset($data) ? $data : NULL);
	...
	public function data_list() {
		...
		$data['items'] = $this->juknis_model->get_list($where,$limit,$offset,$like)->result();
		$tot = $this->juknis_model->get_list_total($where,$like)->row();
		$data['total_items'] =$tot->count;
		$data['page'] = $pagenya;
		$this->load->view('juknis/v_list', $data);
	}

	public function form($id="") {
		if (!$this->ion_auth->logged_in())
		{
			redirect('auth/login?continue='.urlencode(base_url().'ujibanding/juknis'));
		}

		$userid = $this->session->userdata('user_id');

		$data['item']=$this->db->query('select * from ujibanding_juknis where id = "'.$id.'"')->row();
		$data['csrf'] = $this->_get_csrf_nonce();
		$data['message'] = (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));

		$this->template->title = 'Form Hasil Juknis';
		$data['menu'] = 'juknis';
		$data['id'] = $id;
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('juknis/v_form', $data);
		$this->template->publish();
	}
	...
	public function simpan_form($id=false) {
		...
		if($id){
			$file=$this->db->query('select * from ujibanding_juknis where id = "'.$id.'"')->row("file");
			if($_FILES["file"]["name"]<>""){
				if($file <> "" or $file<>null){
					unlink(FCPATH . 'file/juknis/'.$file); 
				}
				
				$upload_path = FCPATH . 'file/juknis/';
			...
			$this->db->update('ujibanding_juknis',$kirim,array('id'=>$id));
		}else{
			if($_FILES["file"]["name"]<>""){				
				$upload_path = FCPATH . 'file/juknis/';
				$config['allowed_types'] = 'pdf';
			...
			$this->db->insert('ujibanding_juknis',$kirim);
		}
		// print_r($this->db->last_query());die();
		redirect('ujibanding/juknis','refresh');
	}

	public function hapus($id)
	{
		$this->db->delete('juknis', array("id"=>$id));
