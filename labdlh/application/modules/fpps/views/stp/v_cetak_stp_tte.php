...
<table width="70%" border="0">
	...
	<tr>
		...
		<td width="5%"></td>
		<td width="90%"></td>
	</tr><tr><td colspan="3"><table border="0" width="100%">';
	$no=0;
	$jum = count($items);
	$kanan = ceil($jum/20);
	$m_kanan = 0;
	foreach ($items as $item) {
		...
		if($kanan==$m_kanan or $m_kanan==0){
			$isi .= '<tr>';
		}

		$isi .= '
		<td align="right" width="20px">'.$no.'</td>
		<td align="left" width="3px">.</td>
		<td width="250px">'.$item->parameter.'</td>
		';
...
$nm_file = str_replace("/","_",$fpps->no_sampel);
if($doc == "0"){
  $this->pdf->Output('STP_'.$nm_file.'.pdf', 'I');
}else{
  $this->pdf->Output('/home/labdlh/public_html/doc/stp/STP_'.$nm_file.'.pdf', 'F');
}
