<div class="box box-default">
	<div class="box-header with-border">
    	...
    	<div style="float: right;">
			<a href="fpps/fpps_pemohon/form/" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> &nbsp;Tambah</a>
		</div>
	</div>
	<div class="box-body">
		<div class="row">
...
function ubah(id)
{
    ...
    $.get(url+'fpps/fpps_pemohon/form/'+id,{}, function(data) {
...
function delet(id){
  	$.ajax({
        url: site+'fpps/fpps_pemohon/hapus/'+id, 
        success: function(r){
	      	json = $.parseJSON(r);
...
function valid(id) {
  ...
  if (answer){
  	$.ajax({
        url: site+'fpps/fpps_pemohon/validasi/'+id, 
        success: function(r){
	      	json = $.parseJSON(r);
	       	if (json.status == 'success') {
	           get_items();
...
function get_items(page){
	...
	var datae = {'page' : page};
	$.post(url+'fpps/fpps_pemohon/fpps_list', $(document.filter.elements).serialize() + '&' + $.param(datae),
	function(data) {
