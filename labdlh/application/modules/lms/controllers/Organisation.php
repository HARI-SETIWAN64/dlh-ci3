...
class Organisation extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model(array('lms/organisation_model'));
	...
	public function index()
	{
		...
		$data['menu'] = 'organisation';
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('organisation/v_index', isset($data) ? $data : NULL);
	...
	public function data_list()
	{
		...
		$data['items'] = $this->organisation_model->get_list($where, $limit, $offset, $like)->result();
		$tot = $this->organisation_model->get_list_total($where, $like)->row();
		$data['total_items'] = $tot->count;
		$data['page'] = $pagenya;
		$this->load->view('organisation/v_list', $data);
	}

	public function tambah() {
		if (!$this->ion_auth->logged_in())
		{
			redirect('auth/login?continue='.urlencode(base_url().'lms/organisation'));
		}
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->template->title = 'Form Organisation';
			$data['menu'] = 'organisation';
			$this->template->description = '';
			$this->template->meta->add('keyword', '');
			$this->template->content->view('organisation/v_tambah', $data);
			$this->template->publish();
		}else{
			if ($_FILES["file"]["name"] <> "") {
			...
			redirect('lms/organisation','refresh');
		}
    }
	...
	public function form($id = "")
	{
		...
		$this->template->title = 'Form Hasil organisation';
		$data['menu'] = 'organisation';
		$data['id'] = $id;
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('organisation/v_form', $data);
		$this->template->publish();
	}
