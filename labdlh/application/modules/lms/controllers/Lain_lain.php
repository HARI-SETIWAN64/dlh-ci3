...
class Lain_lain extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model(array('lms/lain_lain_model'));
	...
	public function index()
	{
		...
		$this->template->title = 'Lain_lain';
		$data['menu'] = 'lain_lain';
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('lain_lain/v_index', isset($data) ? $data : NULL);
	...
	public function data_list()
	{
		...
		$data['items'] = $this->lain_lain_model->get_list($where, $limit, $offset, $like)->result();
		$tot = $this->lain_lain_model->get_list_total($where, $like)->row();
		$data['total_items'] = $tot->count;
		$data['page'] = $pagenya;
		$this->load->view('lain_lain/v_list', $data);
	}

	public function tambah() {
		if (!$this->ion_auth->logged_in())
		{
			redirect('auth/login?continue='.urlencode(base_url().'lms/lain_lain'));
		}
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->template->title = 'Form Lain-lain';
			$data['menu'] = 'lain_lain';
			$this->template->description = '';
			$this->template->meta->add('keyword', '');
			$this->template->content->view('lain_lain/v_tambah', $data);
			$this->template->publish();
		}else{
			if ($_FILES["file"]["name"] <> "") {
			...
			redirect('lms/lain_lain','refresh');
		}
    }
	...
	public function form($id = "")
	{
		...
		$this->template->title = 'Form Hasil lain_lain';
		$data['menu'] = 'lain_lain';
		$data['id'] = $id;
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('lain_lain/v_form', $data);
		$this->template->publish();
	}
