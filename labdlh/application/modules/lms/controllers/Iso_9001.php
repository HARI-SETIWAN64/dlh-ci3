...
class Iso_9001 extends CI_Controller
{
	private $limit = 10;
	function __construct()
	{
		...
		$this->load->model(array('lms/iso_9001_model'));
	...
	public function index()
	{
		...
		$this->template->title = 'iso_9001';
		$data['menu'] = 'iso_9001';
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('iso_9001/v_index', isset($data) ? $data : NULL);
	...
	public function data_list()
	{
		...
		$data['items'] = $this->iso_9001_model->get_list($where, $limit, $offset, $like)->result();
		$tot = $this->iso_9001_model->get_list_total($where, $like)->row();
		$data['total_items'] = $tot->count;
		$data['page'] = $pagenya;
		$this->load->view('iso_9001/v_list', $data);
	}

	public function tambah() {
		if (!$this->ion_auth->logged_in())
		{
			redirect('auth/login?continue='.urlencode(base_url().'lms/iso_9001'));
		}
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->template->title = 'Form ISO-9001';
			$data['menu'] = 'is0_9001';
			$this->template->description = '';
			$this->template->meta->add('keyword', '');
			$this->template->content->view('iso_9001/v_tambah', $data);
			$this->template->publish();
		}else{
			if ($_FILES["file"]["name"] <> "") {
	...
	public function form($id = "")
	{
		...
		$this->template->title = 'Form Hasil Iso_9001';
		$data['menu'] = 'iso_9001';
		$data['id'] = $id;
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('iso_9001/v_form', $data);
		$this->template->publish();
	}
