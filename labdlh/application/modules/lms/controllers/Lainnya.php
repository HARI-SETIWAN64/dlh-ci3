...
class Lainnya extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model(array('lms/lainnya_model'));
	...
	public function index()
	{
		...
		$this->template->title = 'Lainnya';
		$data['menu'] = 'lainnya';
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('lainnya/v_index', isset($data) ? $data : NULL);
	...
	public function data_list()
	{
		...
		$data['items'] = $this->lainnya_model->get_list($where, $limit, $offset, $like)->result();
		$tot = $this->lainnya_model->get_list_total($where, $like)->row();
		$data['total_items'] = $tot->count;
		$data['page'] = $pagenya;
		$this->load->view('lainnya/v_list', $data);
	}

	public function tambah() {
		if (!$this->ion_auth->logged_in())
		{
			redirect('auth/login?continue='.urlencode(base_url().'lms/lainnya'));
		}
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->template->title = 'Form Lainnya';
			$data['menu'] = 'lainnya';
			$this->template->description = '';
			$this->template->meta->add('keyword', '');
			$this->template->content->view('lainnya/v_tambah', $data);
			$this->template->publish();
		}else{
			if ($_FILES["file"]["name"] <> "") {
	...
	public function form($id = "")
	{
		...
		$this->template->title = 'Form Hasil Lainnya';
		$data['menu'] = 'lainnya';
		$data['id'] = $id;
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('lainnya/v_form', $data);
		$this->template->publish();
	}
