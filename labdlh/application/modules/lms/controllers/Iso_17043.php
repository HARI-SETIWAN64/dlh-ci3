...
class Iso_17043 extends CI_Controller
{
	private $limit = 10;
	function __construct()
	{
		...
		$this->load->model(array('lms/iso_17043_model'));
	...
	public function index()
	{
		...
		$this->template->title = 'Iso_17043';
		$data['menu'] = 'iso_17043';
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('iso_17043/v_index', isset($data) ? $data : NULL);
	...
	public function data_list()
	{
		...
		$data['items'] = $this->iso_17043_model->get_list($where, $limit, $offset, $like)->result();
		$tot = $this->iso_17043_model->get_list_total($where, $like)->row();
		$data['total_items'] = $tot->count;
		$data['page'] = $pagenya;
		$this->load->view('iso_17043/v_list', $data);
	}

	public function tambah() {
		if (!$this->ion_auth->logged_in())
		{
			redirect('auth/login?continue='.urlencode(base_url().'lms/iso_17043'));
		}
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->template->title = 'Form Iso_17043';
			$data['menu'] = 'iso_17043';
			$this->template->description = '';
			$this->template->meta->add('keyword', '');
			$this->template->content->view('iso_17043/v_tambah', $data);
			$this->template->publish();
		}else{
			if ($_FILES["file"]["name"] <> "") {
			...
			redirect('lms/iso_17043','refresh');
		}
    }
	...
	public function form($id = "")
	{
		...
		$this->template->title = 'Form Hasil iso_17043';
		$data['menu'] = 'iso_17043';
		$data['id'] = $id;
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('iso_17043/v_form', $data);
		$this->template->publish();
	}
