...
class Iso_17025 extends CI_Controller
{
	private $limit = 10;
	function __construct()
	{
		...
		$this->load->model(array('lms/iso_17025_model'));
	...
	public function index()
	{
		...
		$this->template->title = 'Iso_17025';
		$data['menu'] = 'iso_17025';
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('iso_17025/v_index', isset($data) ? $data : NULL);
	...
	public function data_list()
	{
		...
		$data['items'] = $this->iso_17025_model->get_list($where, $limit, $offset, $like)->result();
		$tot = $this->iso_17025_model->get_list_total($where, $like)->row();
		$data['total_items'] = $tot->count;
		$data['page'] = $pagenya;
		$this->load->view('iso_17025/v_list', $data);
	}

	public function tambah() {
		if (!$this->ion_auth->logged_in())
		{
			redirect('auth/login?continue='.urlencode(base_url().'lms/iso_17025'));
		}

		$this->form_validation->set_rules('nomor_dokumen','Nomor Dokumen', 'required|is_unique[lms_manajemen.nomor_dokumen]|xss_clean',
								array(
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->template->title = 'Form Iso_17025';
			$data['menu'] = 'iso_17025';
			$this->template->description = '';
			$this->template->meta->add('keyword', '');
			$this->template->content->view('iso_17025/v_tambah', $data);
			$this->template->publish();
		}else{
			if ($_FILES["file"]["name"] <> "") {
				$upload_path = FCPATH . 'file/lms_manajemen/';
			...
			$this->db->insert('lms_manajemen', $kirim);

			redirect('lms/iso_17025','refresh');
		}
    }
	...
	public function form($id = "")
	{
		...
		$data['item'] = $this->db->query('select * from lms_manajemen where id = "' . $id . '"')->row();
		$data['csrf'] = $this->_get_csrf_nonce();
		$data['message'] = (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));

		$this->template->title = 'Form Hasil iso_17025';
		$data['menu'] = 'iso_17025';
		$data['id'] = $id;
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('iso_17025/v_form', $data);
		$this->template->publish();
	}
	...
	public function simpan_form($id = false)
	{
		...
		if ($id) {
			$file = $this->db->query('select * from lms_manajemen where id = "' . $id . '"')->row("file");
			if ($_FILES["file"]["name"] <> "") {
				if ($file <> "" or $file <> null) {
					unlink(FCPATH . 'file/lms_manajemen/' . $file);
				}
			...
			$this->db->update('lms_manajemen', $kirim, array('id' => $id));
		} else {
			if ($_FILES["file"]["name"] <> "") {
				$upload_path = FCPATH . 'file/lms_manajemen/';
				$config['allowed_types'] = 'jpg|jpeg|png|pdf|docx|pptx|mp4';
	...
	{
		$this->db->delete('lms_manajemen', array("id" => $id));
	...
	public function status($id, $status)
	{
		...
		$this->db->update('lms_manajemen', $datapost, array('id' => $id));
		$this->data['status'] = 'success';
		echo json_encode($this->data);
	}
