...
class Regulasi extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model(array('lms/regulasi_model'));
	...
	public function index()
	{
		...
		$this->template->title = 'Regulasi';
		$data['menu'] = 'regulasi';
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('regulasi/v_index', isset($data) ? $data : NULL);
	...
	public function data_list()
	{
		...
		if (isset($_POST['nama']) && $_POST['nama'] != NULL) {
			$like = array('nomor_dokumen' => $this->input->post('nama'), 'kategori' => $this->input->post('nama'), 'nama_dokumen' => $this->input->post('nama'));
		}
		...
		$data['items'] = $this->regulasi_model->get_list($where, $limit, $offset, $like)->result();
		$tot = $this->regulasi_model->get_list_total($where, $like)->row();
		$data['total_items'] = $tot->count;
		$data['page'] = $pagenya;
		$this->load->view('regulasi/v_list', $data);
	}

	public function tambah() {
		if (!$this->ion_auth->logged_in())
		{
			redirect('auth/login?continue='.urlencode(base_url().'lms/regulasi'));
		}
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->template->title = 'Form Regulasi';
			$data['menu'] = 'regulasi';
			$this->template->description = '';
			$this->template->meta->add('keyword', '');
			$this->template->content->view('regulasi/v_tambah', $data);
			$this->template->publish();
		}else{
			if ($_FILES["file"]["name"] <> "") {
	...
	public function form($id = "")
	{
		...
		$this->template->title = 'Form Hasil Regulasi';
		$data['menu'] = 'regulasi';
		$data['id'] = $id;
		$this->template->description = '';
		$this->template->meta->add('keyword', '');
		$this->template->content->view('regulasi/v_form', $data);
		$this->template->publish();
	}
