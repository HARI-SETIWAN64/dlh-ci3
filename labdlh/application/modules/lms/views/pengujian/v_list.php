  ...
  <table class="table table-bordered table-striped">
    ...
    <?php foreach ($items as $row): ?>
      <tr>
        ...
        <td>
          <?php if ($row->file <> "") { ?>
            <a href="<?php echo base_url() . 'file/lms_teknis/' . $row->file ?>" class="btn btn-xs btn-success"
        ...
        <?php if ($this->ion_auth->in_group(array('analis', 'admin'))) { ?>
          <td>
            ...
            <a href="lms/pengujian/form/<?php echo $row->id; ?>" class="btn btn-success btn-xs" title="Edit Dokumen"><i
...
<script>
  function status(id, status) {
    ...
    if (answer) {
      $.ajax({
        url: site + 'lms/pengujian/status/' + id + '/' + status,
        success: function (r) {
          json = $.parseJSON(r);
